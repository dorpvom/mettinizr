...
def test_order_bun(mock_app, app_setup):
    app_setup.mett_store.create_account(MockUser.email)
    app_setup.mett_store.create_order()
    response = mock_app.get('/order')
    assert b'132.0 g of mett' in response.data
...
def test_show_purpose_page(mock_app, app_setup):
    app_setup.mett_store.create_account(MockUser.email)
...
def test_state_purchase(mock_app, app_setup):
    app_setup.mett_store.create_account(MockUser.email)
    assert not app_setup.mett_store.list_purchases()

    response = mock_app.post('/order/purchase', data={'amount': 7.32, 'purpose': 'For testing'})
    assert response.status_code == 200

    purchases = app_setup.mett_store.list_purchases()
    assert purchases
